...
//	KeepLoss(Lmat, N, keepN, &vBigL[0], &vBigLIdx[0]);
	...
	float integral = Lmat[0] * vBigP[0] * vBigT[0];
	float Cuvw = 0;
	...
	for(int level = 0; level < levelNum; ++level)
		for(int iIdx = 0; iIdx < (1<<level); ++iIdx)
			for(int jIdx = 0; jIdx < (1<<level); ++jIdx)
			{
				...
				float L1 = GetCoef(&Lmat[0], level, iIdx, jIdx, 1, res);
				float L2 = GetCoef(&Lmat[0], level, iIdx, jIdx, 2, res);
				float L3 = GetCoef(&Lmat[0], level, iIdx, jIdx, 3, res);
				...
				integral += Cuvw * (
					...
					P3 * L2 * T1);
				//case 3
				psumL = GetCoefPSum(&Lmat[0], level, iIdx, jIdx, res);
