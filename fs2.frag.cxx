...
const float m_yita2 = 1.69; //yita^2
const int m_cubeRes = 32;

// const float m_SSSampleNum = 8.0; // 16.0;
const vec4 sigmat0123 = vec4(0, 0.053569, 0.16071, 0.32141);
const vec4 sigmat4567 = vec4(0.53569, 0.80353, 1.1249, 1.5);
...
void main(void)
{
		...
		gl_FragData[0] = gl_FragData[1] = vec4(0.0);
		vec3 Wi = Texcoord2Dir(texCoord);
		float Cos = dot(Ni, Wi);
		if(Cos > 0.0)
		{
			float Si1 = texture2D(NiSiSampler, texCoord).a * (Cos / sqrt(1.0 - (1.0 - Cos * Cos) / m_yita2));
			//by outer controls, only draw at vis==1 places. so here needn't care about visb issue.
			gl_FragData[0] = exp(- sigmat0123 * (intrudepdf.x + Si1)) / intrudepdf.y;// / m_SSSampleNum;
			gl_FragData[1] = exp(- sigmat4567 * (intrudepdf.x + Si1)) / intrudepdf.y;// / m_SSSampleNum;
		}
}
