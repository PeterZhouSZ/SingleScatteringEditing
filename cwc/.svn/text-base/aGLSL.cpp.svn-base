...
int CheckGLError(char *file, int line)
{
	...
	while (glErr != GL_NO_ERROR) {
		const GLubyte* p = gluErrorString(glErr);
		if(p == NULL) //yk	: cout crashes at <<null
			cout << "GL Error #" << glErr;
		else
			cout << "GL Error #" << glErr << "(" << p << ") " << " in File " << file << " at line: " << line << endl;
		retCode = 1;
		glErr = glGetError();
	}
	return retCode;
...
int aShaderProgram::load(char* filename)
{
	...
	while (file.good())
	{
		ShaderSource[i++] = (GLubyte)file.get();       // get character from file
...
int aShaderProgram::load(char* filename)
{
	...
	file.read((char*)ShaderSource, len);  
	ShaderSource[len] ='\0';  // len isn't always strlen cause some characters are stripped in ascii read...
                            // it is important to 0-terminate the real length later, len is just max possible value...
	//unsigned int i=0;
	//while (file.good())
...
// ----------------------------------------------------------------------------
 ...
 aShaderObject* DefaultShaderSetup(char* vertex, char* fragment, aShaderObject* _DefaultShader, aVertexShader* _DefaultVertexShader, aFragmentShader* _DefaultFragmentShader)
 {
	 _DefaultShader = new aShaderObject;
	 if (vertex!=0) 
		 _DefaultVertexShader = new aVertexShader;
	 if (fragment!=0) 
		 _DefaultFragmentShader = new aFragmentShader;
	 ...
	 if (vertex!=0)
		 ...
		 { 
			 ...
			 return NULL;//yk exit(-1);  // if file is missing: major error, better exit
		 }
	...
	// Add to object    
	if (vertex!=0) 
		_DefaultShader->addShader(_DefaultVertexShader);
	if (fragment!=0) 
		_DefaultShader->addShader(_DefaultFragmentShader); 
 ...
 void DefaultCleanup(aShaderObject* _DefaultShader, aVertexShader* _DefaultVertexShader, aFragmentShader* _DefaultFragmentShader)
 {
	 // free default Shaders if defined
	 if (_DefaultShader!=0) 
		 delete _DefaultShader; 
	 _DefaultShader=0;
	 if (_DefaultVertexShader!=0) 
		 delete _DefaultVertexShader; 
	 _DefaultVertexShader=0;
	 if (_DefaultFragmentShader!=0) 
		 delete _DefaultFragmentShader; 
